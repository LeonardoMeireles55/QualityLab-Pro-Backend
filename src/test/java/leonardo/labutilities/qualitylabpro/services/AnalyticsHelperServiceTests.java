...
class AnalyticsHelperServiceTests {
	...
	void setUp() {
		...
		analyticsHelperService =
				new AnalyticsHelperService(genericAnalyticsRepository, rulesValidatorComponent) {
					@Override
					public List<GenericValuesRecord> findAnalyticsByNameAndLevel(Pageable pageable,
							String name, String level) {
						return genericAnalyticsRepository.findAllByNameAndLevel(pageable, name,
								level);
					}
					...
					public List<GenericValuesRecord> findAllAnalyticsByNameAndLevelAndDate(
						...
						return genericAnalyticsRepository.findAllByNameAndLevelAndDateBetween(name,
	...
	void shouldValidateRulesProcessedByRulesValidatorComponent() {
		...
		List<GenericAnalytics> genericAnalytics = records.stream()
				.map(values -> new GenericAnalytics(values, rulesValidatorComponent)).toList();
	...
	void findAllAnalyticsByNameAndLevelAndDate_WithDateRange_ShouldReturnFilteredRecords() {
		...
		when(genericAnalyticsRepository.findAllByNameAndLevelAndDateBetween(eq(name), eq(level),
	...
	void isAnalyticsNonExistent_WithNonExistentRecord_ShouldReturnTrue() {
		...
		when(genericAnalyticsRepository.existsByDateAndLevelAndName(record.date(), record.level(),
