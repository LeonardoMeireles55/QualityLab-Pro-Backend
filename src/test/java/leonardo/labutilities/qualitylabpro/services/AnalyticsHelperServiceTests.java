...
class AnalyticsHelperServiceTests {
    ...
    void setUp() {
        ...
        analyticsHelperService = new AnalyticsHelperService(genericAnalyticsRepository, rulesValidatorComponent) {
            @Override
            public List<GenericValuesRecord> findAnalyticsByNameAndLevel(Pageable pageable, String name, String level) {
                return genericAnalyticsRepository.findAllByNameAndLevel(pageable, name, level);
            }
            ...
            public List<GenericValuesRecord> findAllAnalyticsByNameAndLevelAndDate(
                ...
                return genericAnalyticsRepository.findAllByNameAndLevelAndDateBetween(
                        name, level, dateStart, dateEnd, PageRequest.of(0, 200));
            }
        };
    }
    ...
    void saveNewAnalyticsRecords_WithValidRecords_ShouldSaveSuccessfully() {
        List<GenericValuesRecord> records = createSampleRecordList();
        when(genericAnalyticsRepository.existsByDateAndLevelAndName(any(), any(), any())).thenReturn(false);
    ...
    void saveNewAnalyticsRecords_WithDuplicateRecords_ShouldThrowException() {
        List<GenericValuesRecord> records = createSampleRecordList();
        when(genericAnalyticsRepository.existsByDateAndLevelAndName(any(), any(), any())).thenReturn(true);
    ...
    void findAnalyticsByNameAndLevel_WithFilters_ShouldReturnFilteredRecords() {
        ...
        List<GenericValuesRecord> result = analyticsHelperService.findAnalyticsByNameAndLevel(pageable, name, level);
    ...
    void findAllAnalyticsByNameAndLevelAndDate_WithDateRange_ShouldReturnFilteredRecords() {
        ...
        when(genericAnalyticsRepository.findAllByNameAndLevelAndDateBetween(
                eq(name), eq(level), eq(startDate), eq(endDate), any(Pageable.class)))
                .thenReturn(expectedRecords);

        List<GenericValuesRecord> result = analyticsHelperService.findAllAnalyticsByNameAndLevelAndDate(
    ...
    void isAnalyticsNonExistent_WithNonExistentRecord_ShouldReturnTrue() {
        ...
        when(genericAnalyticsRepository.existsByDateAndLevelAndName(
                record.date(), record.level(), record.name())).thenReturn(false);
    ...
    void isAnalyticsNonExistent_WithExistentRecord_ShouldReturnFalse() {
        GenericValuesRecord record = createSampleRecord();
        when(genericAnalyticsRepository.existsByDateAndLevelAndName(
                record.date(), record.level(), record.name())).thenReturn(true);
    ...
    void findGroupedAnalyticsByLevel_WithValidInputs_ShouldReturnGroupedRecords() {
        ...
        when(genericAnalyticsRepository.findAllByNameAndDateBetweenGroupByLevel(
                eq(name), eq(startDate), eq(endDate), any(Pageable.class)))
                .thenReturn(records);

        List<GroupedValuesByLevel> result = analyticsHelperService.findGroupedAnalyticsByLevel(
                name, startDate, endDate);

        assertNotNull(result);
        assertFalse(result.isEmpty());
        verify(genericAnalyticsRepository).findAllByNameAndDateBetweenGroupByLevel(
                eq(name), eq(startDate), eq(endDate), any(Pageable.class));
    }
}
