...
public abstract class GenericAnalyticsController {
    ...
    public ResponseEntity<List<GenericResultsGroupByLevel>> getGroupedByLevel(
        ...
        List<GenericResultsGroupByLevel> groupedData = analyticsHelperService.getGroupedResults(name, startDate,
                endDate);
    ...
    public ResponseEntity<List<MeanAndStandardDeviationRecordGroupByLevel>> getMeanAndDeviationGrouped(
            ...
            @RequestParam("startDate") LocalDateTime startDate,
            @RequestParam("endDate") LocalDateTime endDate) {
        List<MeanAndStandardDeviationRecordGroupByLevel> groupedData = analyticsHelperService
                .generateMeanAndStandardDeviationGrouped(name, startDate, endDate);
        return ResponseEntity.ok(groupedData);
    }
    ...
    public ResponseEntity<CollectionModel<EntityModel<GenericValuesRecord>>> getAllAnalyticsHateoas(
        ...
        List<EntityModel<GenericValuesRecord>> resultModels = resultsList
                .stream()
                .map(result -> EntityModel.of(result, linkTo(getClass()).slash(result.id()).withSelfRel()))
    ...
    public ResponseEntity<CollectionModel<EntityModel<GenericValuesRecord>>> getAllAnalyticsByNameOrderByDate(
        ...
        return ResponseEntity.ok(
                CollectionModel.of(
                        resultModels,
                        linkTo(
                                methodOn(getClass()).getAllAnalyticsByNameOrderByDate(name, pageable)).withSelfRel()));
    }
